...
${(() => {
  ...
  if (/new well|navin vihir|farm pond|plastic lining/i.test(s)) return '      - Land MUST be Jirayat. Check BOTH the top Form 7 summary AND the Form 12 crop details at the bottom. The MOST RECENT year in Form 12 is the final truth. If the most recent year shows "Irrigated" crops or a well ("vihir"), reject it (it is currently Bagayat). Otherwise, pass.';
  if (/old well|juni vihir|boring|pump set|electricity connection/i.test(s)) return '      - Land MUST be Bagayat. Check BOTH the top Form 7 summary AND the Form 12 crop details at the bottom. The MOST RECENT year in Form 12 is the final truth. If the most recent year shows "Irrigated" crops or a well ("vihir"), consider it Bagayat and pass it. Reject ONLY if the most recent year is purely Non-Irrigated (Jirayat).';
