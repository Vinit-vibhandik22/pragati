...
${(() => {
  ...
  if (/new well|navin vihir|farm pond|plastic lining/i.test(s)) return '      - CRITICAL OVERRIDE: Land MUST be Jirayat. Do not trust the top Form 7 summary if it contradicts the bottom Form 12 crop details. Look at the MOST RECENT year in Form 12. If it shows ANY "Irrigated" crops (e.g. 1.0000) or a well ("vihir"), reject it (it is currently Bagayat). Otherwise, pass.';
  if (/old well|juni vihir|boring|pump set|electricity connection/i.test(s)) return '      - CRITICAL OVERRIDE: Land MUST be Bagayat. Do not trust the top Form 7 summary if it says Non-Irrigated. Look at the MOST RECENT year in Form 12 at the bottom. If it shows ANY "Irrigated" crops (e.g. 1.0000) or a well ("vihir"), you MUST consider it Bagayat and pass it. Reject ONLY if the most recent year in Form 12 is purely Non-Irrigated (Jirayat).';
