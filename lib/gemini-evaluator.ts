...
${(() => {
  ...
  if (/new well|navin vihir|farm pond|plastic lining/i.test(s)) return '      - CRITICAL: Land MUST be Jirayat. IGNORE the top Form 7 summary box. IGNORE all older years in Form 12. ONLY look at the MOST RECENT year in Form 12. Do NOT match Khata numbers. If the most recent year shows ANY irrigated crops or well (vhir/vihir), reject. Otherwise pass.';
  if (/old well|juni vihir|boring|pump set|electricity connection/i.test(s)) return '      - CRITICAL: Land MUST be Bagayat. IGNORE the top Form 7 summary box. IGNORE all older years in Form 12. ONLY look at the MOST RECENT year in Form 12. Do NOT match Khata numbers. If the most recent year shows ANY irrigated crops or well (vhir/vihir) anywhere, classify as Bagayat and PASS. Reject ONLY if the most recent year has zero irrigated crops and no well.';
